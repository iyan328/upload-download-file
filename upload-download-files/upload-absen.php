...
<body>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">
                            Upload Absensi <small><?php echo $nama?></small>
                        </h1>
                        <ol class="breadcrumb">
                            <li>
							...
							<li class="active">
                                <i class="fa fa-file"></i> Upload Absensi
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    <div class="col-lg-6">
                        <form role="form" action="" method="post" enctype="multipart/form-data">
							...
							<div class="form-group">
								<label>Absensi</label>
								<input name="absensi" type="file" required>
    ...
    <script src="js/plugins/morris/morris-data.js"></script>
			...
			if(isset($_POST['upload'])){
				...
				$file_name		= $_FILES['absensi']['name'];
				$value			= explode('.', $file_name);
				$file_ext		= strtolower(end($value));
				$file_size		= $_FILES['absensi']['size'];
				$file_tmp		= $_FILES['absensi']['tmp_name'];

				if(in_array($file_ext, $allowed_ext) === true){
					if($file_size < 1e+7){
						$lokasi = 'files/'.$file_name;
						move_uploaded_file($file_tmp, $lokasi);
						$in = mysql_query("INSERT INTO upload VALUES(NULL, '$nim', '$date', NULL, NULL, '$file_name', NULL, NULL, '$lokasi')");
						if($in){
							echo "<script> alert('Upload berhasil'); document.location='../mhs/mahasiswa.php'</script>";
						}else{
							echo "<script> alert('Upload Gagal'); document.location='../upload-download-files/upload-absen.php'</script>";
						}
					}else{
