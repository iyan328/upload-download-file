...
<?php
	...
	if(isset($_GET['kode'])){
		...
		echo "<script>alert('Perusahaan Belum Dipilih');document.location='../mhs/kerjasama.php'</script>";
	}
...
<body>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
							...
							<li>
                                <i class="fa fa-pencil"></i> <a href="../mhs/kerjasama.php"> Jalur Kerjasama </a>
                            </li>
							<li class="active">
                    ...
                    <div class="col-lg-4">
                        <form role="form" action="" method="post" enctype="multipart/form-data">
							...
							<div class="form-group">
                                ...
                                <input name="prodi" class="form-control" id="disabledInput" type="text" value = "<?php echo $prodi?>" readonly>
                            </div>
							<div class="form-group">
							...
							<div class="form-group">
                                ...
                                <input name="jalur" class="form-control" id="disabledInput" type="text" value="kerjasama" readonly>
                            </div>
							<div class="form-group">
								<label>CV</label>
    ...
    <script src="js/plugins/morris/morris-data.js"></script>
			...
			$temp_tgl="create table temp_tanggal (tgl_masuk date, tgl_keluar date)";
			$cek_tgl=mysql_query($temp_tgl);
			
			$insert_tgl="insert into temp_tanggal values('$tgl_masuk', '$tgl_keluar')";
			$cek_insert=mysql_query($insert_tgl);
			
			$bulan_masuk="select month(tgl_masuk) as tgl from temp_tanggal";
			$cek_masuk=mysql_query($bulan_masuk);
			$data_masuk=mysql_fetch_array($cek_masuk);
			
			$bulan_keluar="select month(tgl_keluar) as tgl from temp_tanggal";
			$cek_keluar=mysql_query($bulan_keluar);
			$data_keluar=mysql_fetch_array($cek_keluar);
			
			if(($data_keluar['tgl']-$data_masuk['tgl'])<4){
				echo "<script> alert('Minimal Magang 4 bulan');</script>";
			}else {
				...
				if(in_array($file_ext, $allowed_ext) === true && in_array($file_ext1, $allowed_ext) === true){
					if($file_size < 1044070 && $file_size1 < 1044070){
						...
						$in = mysql_query("update magang set prodi = '$prodi', perusahaan='$perusahaan', tgl_masuk='$tgl_masuk', tgl_keluar='$tgl_keluar', jalur='$jalur', s_seleksi='Belum di Seleksi', verifikasi='Belum', cv='$file_name', transkip='$file_name1', filecv='$lokasi', filetranskip='$lokasi1', keterangan=NULL where nim='$nim'");
						//$in = mysql_query("UPDATE magang set '$nim','$prodi', '$perusahaan', '$tgl_masuk', '$tgl_keluar', '$jalur', 'Belum di Seleksi','Belum', '$file_name', '$file_name1', '$lokasi', '$lokasi1'");
						if($in){
							echo "<script> document.location='../mhs/mahasiswa.php'</script>";
						}else{
			...
			$del_temp="drop table temp_tanggal";
			$cek_del=mysql_query($del_temp);
			if (isset($_POST['batal'])){
		echo "<script> document.location='../mhs/kerjasama.php'</script>";
